...
int input_read_parameters(
  ...
  char param_name[_ARGUMENT_LENGTH_MAX_];
  ...
  double n_cor=0.;
  double c_cor=0.;
  int N_coupling_idr=0,N_idr=0;
  double stat_f_idr = 7./8.,xi_idr=0.,T_nu0n=0.;
  double param_value;
  ...
  if (class_none_of_three(flag1,flag2,flag3)) {  //AD: Default neutrino species is now _idr//SG-override//
    pba->Omega0_ur = 0.0; // 3.046*7./8.*pow(4./11.,4./3.)*pba->Omega0_g; //
  }
  else {
  ...
  /* AD: stat_f_idr, xi_idr and only one of N_idr and N_dg are read from the file. All neutrinos are assumed to be at the temperature xi_idr*T_cmb. The idr density is then inferred. 
     N.B.- N_idr or N_dg are not number of *effective* neutrino species. They are just number of neutrino species and can only be integers. But we need N_eff as a free parameter, no? */

  class_read_double("stat_f_idr",stat_f_idr);

  class_read_int("N_idr",pba->N_idr); //AD
             
  class_read_double("xi_idr",pba->xi_idr);

  pba->T_idr = pow(pba->N_idr/stat_f_idr*(7./8.)*pow(pba->xi_idr,4.), (1./4.)) * pba->T_cmb;
  if (input_verbose > 1){
      printf("You passed N_idr = %d and xi_idr= %e\n", pba->N_idr, pba->xi_idr);
  }

  N_coupling_idr = pba->N_idr;
  pba->Omega0_idr = pba->N_idr * stat_f_idr * pow(pba->xi_idr, 4.) * pba->Omega0_g;
  ...
  /*if (pba->Omega0_idm_dr > 0.) {
    ...
    //class_read_list_of_doubles_one_of_two("b_dark","b_idr",pth->b_idr); //AD
    class_call(parser_read_list_of_doubles(pfc,"b_dark",&entries_read,&(pointer1),&flag1,errmsg),
                 errmsg,
                 errmsg);
    if (flag1 == _FALSE_) {
        class_call(parser_read_list_of_doubles(pfc,"b_idr",&entries_read,&(pointer1),&flag1,errmsg),
                    errmsg,
                    errmsg);
    }
    class_test(entries_read > N_coupling_idr, errmsg, "The number of neutrino self-couplings should be equal to or less than the number of species."); //AD
    for(n=0; n<entries_read; n++){
       pth->b_idr[n] = pointer1[n];
    }
    free(pointer1);
    if(entries_read < N_coupling_idr) {
      printf("You entered less than %d neutrino self-couplings. Others will be assumed to be zero.\n", pba->N_idr);
      for(n=entries_read; n<N_coupling_idr; n++){
         pth->b_idr[n] = 0.;
      }
    }*/
  ...
  /** Neutrino self-interaction parameters */
    ...
    if (pba->N_idr != 0) { //AD
       // Reading neutrino interaction strength (same for all states)
       T_nu0n = pow(pba->T_cmb*1.e7*pba->xi_idr*8.6173e-11, pth->nindex_idm_dr+1);
       for (n=0; n<pba->N_idr; n++){
          sprintf(str,"%d",n+1);
          strcpy(param_name,"log10_G_eff_");
          strcat(param_name,str);
          class_call(parser_read_double(pfc,param_name,&param1,&flag1,errmsg),
                errmsg,
                errmsg);
       ...
       if (input_verbose > 2){
          printf("Neutrino interaction strengths --\n");
          for (n=0; n<pba->N_idr; n++){
             printf(" Species %d -> b_idr[%d] = %e Mpc^-1\n",n+1,n+1,pth->b_idr[n]);
          }
       }
       
       // Reading neutrino interaction params all in a list
       //class_call(parser_read_list_of_doubles(pfc,"G_eff",&entries_read,&(pointer1),&flag1,errmsg),
         //        errmsg,
         //        errmsg);
       
       //class_call(parser_read_list_of_doubles(pfc,"b_idr",&entries_read,&(pointer1),&flag2,errmsg),
         //        errmsg,
         //        errmsg);
       //class_test(flag1 == _TRUE_ && flag2 == _TRUE_, errmsg, "You passed both G_eff and b_idr as input. Choose one!");
       
       //if (flag1 == _TRUE_){
         // class_test(entries_read > N_coupling_idr, errmsg, "The number of neutrino self-couplings should be equal to or less than the number of species.");
          //T_nu0n = pow(pba->T_cmb*1.e7*pba->xi_idr*8.6173e-11, pth->nindex_idm_dr+1);
          //for(n=0; n<entries_read; n++){
            //  pth->b_idr[n] = (pow(pointer1[n],2)*T_nu0n/pba->Omega0_idr/pow(pba->h,2)/1.e7)*1.543e35; // in Mpc^-1
          //}
          //free(pointer1);
       //}
       //else if (flag2 == _TRUE_) {
     ...
     /* Fill up other couplings with zeros */  // Commented out as this may conflict with the new part of the input.
     //if(entries_read < N_coupling_idr) {
       //printf("You entered less than %d neutrino self-couplings. Others will be assumed to be zero.\n", pba->N_idr);
       //for(n=entries_read; n<N_coupling_idr; n++){
        //  pth->b_idr[n] = 0.;
...
int input_default_params(
  ...
  pba->Omega0_g = (4.*sigma_B/_c_*pow(pba->T_cmb,4.)) / (3.*_c_*_c_*1.e10*pba->h*pba->h/_Mpc_over_m_/_Mpc_over_m_/8./_PI_/_G_);
  pba->Omega0_ur = 0.0;   //3.046*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
  //pba->Omega0_idr = 3.046*7./8.*pow(4./11.,4./3.)*pba->Omega0_g; //AD: _idr is the default neutrino species
  pba->N_idr = 3;
  pba->xi_idr = pow(4./11., 1./3.);
  pba->Omega0_idm_dr = 0.0;
  //pba->T_idr = pow(3./stat_f_idr*(7./8.)/pow(11./4.,(4./3.)),(1./4.)) * pba->T_cmb;
