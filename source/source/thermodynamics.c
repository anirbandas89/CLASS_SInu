...
int thermodynamics_at_z(
  ...
  int n;
  double x0;
  int N_coupling_idr = pba->N_idr; //AD: This is the maximum number of diagonal couplings in the neutrino sector
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    if(pba->has_idr == _TRUE_){ //AD: All _idm_dr lines are commented out
      ...
      for(n=0; n<N_coupling_idr; n++){
         pvecthermo[pth->index_th_dmu_idr+n] = pth->b_idr[n]*pow((1.+z)/1.e7,pth->nindex_idm_dr)*pba->Omega0_idr*pow(pba->h,2);
      }
  ...
  for (index_tau=0; index_tau < pth->tt_size; index_tau++) {
    ...
    if(pba->has_idr == _TRUE_) {
      ...
      for(n_idr=0; n_idr<pba->N_idr; n_idr++){
         pth->thermodynamics_table[index_tau*pth->th_size+pth->index_th_dmu_idr+n_idr] =
               pth->b_idr[n_idr]*pow((1.+pth->z_table[index_tau])/1.e7,pth->nindex_idm_dr)*pba->Omega0_idr*pow(pba->h,2);
      }
    }
  }
...
int thermodynamics_indices(
  ...
  if(pba->has_idr == _TRUE_){ //AD: We need N(N+1)/2 no of couplings
    pth->index_th_dmu_idr = index;
    index += pba->N_idr;
    pth->index_th_tau_idr = index;
    index++;
