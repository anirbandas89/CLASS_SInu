...
int background_functions(
  ...
  if (pba->has_idr == _TRUE_) { //AD
   for (n_idr=0; n_idr<pba->N_idr; n_idr++) {
    pvecback[pba->index_bg_rho_idr+n_idr] = pba->Omega0_idr * pow(pba->H0,2) / pow(a_rel,4)/pba->N_idr;
    rho_tot += pvecback[pba->index_bg_rho_idr+n_idr];
    p_tot += (1./3.) * pvecback[pba->index_bg_rho_idr+n_idr];
    rho_r += pvecback[pba->index_bg_rho_idr+n_idr];
   }
  }
...
int background_init(
  ...
  if (pba->background_verbose > 0) {
    ...
    if ((pba->N_ncdm > 0) || (pba->Omega0_idr != 0.))  {
      ...
      if (pba->Omega0_idr != 0.) {
        Neff += pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g;
        N_dark = pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g - 3.046;
        printf(" -> dark radiation Delta Neff %g\n",N_dark);
      }
...
int background_indices(
  ...
  class_define_index(pba->index_bg_rho_idr,pba->has_idr,index_bg,pba->N_idr);
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_ur],pba->has_ur,storeidx);
      if (pba->has_idr == _TRUE_){ //AD
        for (n=0; n<pba->N_idr; n++){
          class_store_double(dataptr,pvecback[pba->index_bg_rho_idr+n],_TRUE_,storeidx);
        }
      }
...
int background_output_budget(
  ...
  if(pba->background_verbose > 1){
    ...
    if(pba->has_idr){
      _class_print_species_("Massless neutrinos",idr);
      budget_radiation+=pba->Omega0_idr;
      budget_neutrino+=pba->Omega0_idr; //AD: We are going to use idr as neutrino
    }
