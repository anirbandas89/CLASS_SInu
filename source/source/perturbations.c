    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          for(n_idr = 0; n_idr < pba->N_idr; n_idr++){ //AD
             class_store_double(dataptr,tk[ppt->index_tp_delta_idr+n_idr],ppt->has_source_delta_idr,storeidx);
          }
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          for(n_idr = 0; n_idr < pba->N_idr; n_idr++){ //AD
             class_store_double(dataptr,tk[ppt->index_tp_theta_idr+n_idr],ppt->has_source_theta_idr,storeidx);
          }
          if (pba->has_ncdm == _TRUE_){
            for (n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++){
...
int perturb_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      for(n_idr = 0; n_idr < pba->N_idr; n_idr++){ //AD
         sprintf(tmp,"d_idr[%d]",n_idr);
         class_store_columntitle(titles,tmp,pba->has_idr);
      }
      if (pba->has_ncdm == _TRUE_) {
        for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_idm_dr == _TRUE_)
          ppt->has_source_delta_idm_dr = _FALSE_; //AD
      ...
      class_define_index(ppt->index_tp_delta_idr,  ppt->has_source_delta_idr, index_type, pba->N_idr); //AD
      ...
      class_define_index(ppt->index_tp_theta_idr,  ppt->has_source_theta_idr, index_type, pba->N_idr);
...
int perturb_solve(
  ...
  int n_ncdm, n_idr,is_early_enough;
  ...
  class_test(k == 0.,
             ppt->error_message,
             "stop to avoid division by zero");
  //fprintf(stderr,"Evolving wavenumber %e \n",k);
  ...
  for (index_interval=0; index_interval<interval_number; index_interval++) {
    ...
    //fprintf(stderr,"call to evolver \n");
  ...
  //fprintf(stderr,"evolver completed \n");
...
int perturb_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      for(n_idr = 0; n_idr < pba->N_idr; n_idr++){ //AD
         sprintf(tmp,"delta_idr[%d]",n_idr);
         class_store_columntitle(ppt->scalar_titles,tmp,pba->has_idr);
         sprintf(tmp,"theta_idr[%d]",n_idr);
         class_store_columntitle(ppt->scalar_titles,tmp,pba->has_idr);
         if ((pba->has_idr == _TRUE_)&&(ppt->idr_nature == idr_free_streaming))
            sprintf(tmp,"shear_idr[%d]",n_idr);
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    /* interacting dark radiation */
    /* AD: The idr section of the perturb vector looks like: {{all l=0}, {all l=1}, {all l=2}, ..., {all lmax_idr}} */
    if (pba->has_idr == _TRUE_){ //AD
      if(ppw->approx[ppw->index_ap_rsa_idr]==(int)rsa_idr_off) {
        ppv->N_idr = pba->N_idr;
        class_define_index(ppv->index_pt_delta_idr,_TRUE_,index_pt,pba->N_idr); /* density of interacting dark radiation */
        class_define_index(ppv->index_pt_theta_idr,_TRUE_,index_pt,pba->N_idr); /* velocity of interacting dark radiation */
        if (ppt->idr_nature == idr_free_streaming){
           class_define_index(ppv->index_pt_shear_idr,_TRUE_,index_pt,pba->N_idr); /* shear of interacting dark radiation */
           ppv->l_max_idr = ppr->l_max_idr;
           idx = (ppv->l_max_idr-2)*pba->N_idr;
           class_define_index(ppv->index_pt_l3_idr,_TRUE_,index_pt,idx); /* additional momenta in Boltzmann hierarchy (beyond l=0,1,2,3) */
         }
      }
    }
  ...
  if (_scalars_) {
    ...
    if (pba->has_idr == _TRUE_) {
      ...
      if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off){
        if (ppt->idr_nature == idr_free_streaming){
          //for(n_idr=0; n_idr<ppv->N_idr; n_idr++){ //AD
            for (index_pt=ppv->index_pt_l3_idr; index_pt <= ppv->index_pt_delta_idr + pba->N_idr*(ppv->l_max_idr+1) - 1; index_pt++){
              ppv->used_in_sources[index_pt]=_FALSE_;
            }
          //}
        }
        ...
        if (pba->has_idr == _TRUE_){
          ...
          if (ppw->approx[ppw->index_ap_rsa_idr]==(int)rsa_idr_off){
           ...
           for(n_idr=0; n_idr<ppv->N_idr; n_idr++){ //AD
            ...
            ppv->y[ppv->index_pt_delta_idr+n_idr] =
              ppw->pv->y[ppw->pv->index_pt_delta_idr+n_idr];

            ppv->y[ppv->index_pt_theta_idr+n_idr] =
              ppw->pv->y[ppw->pv->index_pt_theta_idr+n_idr];

            if (ppt->idr_nature == idr_free_streaming){

                ppv->y[ppv->index_pt_shear_idr+n_idr] =
                  ppw->pv->y[ppw->pv->index_pt_shear_idr+n_idr];

                ppv->y[ppv->index_pt_l3_idr+n_idr] =
                  ppw->pv->y[ppw->pv->index_pt_l3_idr+n_idr];

                for (l=4; l <= ppv->l_max_idr; l++)
                  ppv->y[ppv->index_pt_delta_idr+n_idr+l*pba->N_idr] =
                    ppw->pv->y[ppw->pv->index_pt_delta_idr+n_idr+l*pba->N_idr];
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    if (pba->has_idr == _TRUE_) { //AD
        for (n_idr=0; n_idr<pba->N_idr; n_idr++){
         rho_r += ppw->pvecback[pba->index_bg_rho_idr + n_idr];
         rho_nu += ppw->pvecback[pba->index_bg_rho_idr + n_idr];
        }
    }
    ...
    */
    ...
    if (pba->has_idr == _TRUE_){
     for(n_idr=0; n_idr<pba->N_idr; n_idr++){ //AD
      ppw->pv->y[ppw->pv->index_pt_delta_idr+n_idr] = delta_ur;
      ppw->pv->y[ppw->pv->index_pt_theta_idr+n_idr] = theta_ur;
      if (ppt->idr_nature == idr_free_streaming){
          ppw->pv->y[ppw->pv->index_pt_shear_idr+n_idr] = shear_ur;
          ppw->pv->y[ppw->pv->index_pt_l3_idr+n_idr] = l3_ur;
      }
     }
    }
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == newtonian) {
      ...
      if ((pba->has_idr)&&(ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_on)){
        ...
        //class_call(perturb_rsa_idr_delta_and_theta(ppr,pba,pth,ppt,k,y,a_prime_over_a,ppw->pvecthermo,ppw),
                  // ppt->error_message,
                  // ppt->error_message); //*SG - no call to perturb_rsa_idr_delta_and_theta
      }
    ...
    if (ppt->gauge == synchronous) {
      ...
      if ((pba->has_idr==_TRUE_)&&(ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_on)) {
        ...
        for(n_idr=0; n_idr<pba->N_idr; n_idr++){ //AD
           ppw->rho_plus_p_theta += 4./3.*ppw->pvecback[pba->index_bg_rho_idr+n_idr]*(-0.5*ppw->pvecmetric[ppw->index_mt_h_prime]);
            //ppw->rho_plus_p_theta += 4./3.*ppw->pvecback[pba->index_bg_rho_idr+n_idr]*ppw->rsa_theta_idr;
            //*SG - for synchronous gauge ppw->rsa_theta_idr = -0.5*ppw->pvecmetric[ppw->index_mt_h_prime];
        }
      ...
      if ((pba->has_idm_dr == _TRUE_)&&(ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_on)){ //AD!
...
int perturb_total_stress_energy(
  ...
  double theta_ur=0.;
  double shear_ur=0.;
  double delta_idr[pba->N_idr];
  double theta_idr[pba->N_idr];
  double shear_idr[pba->N_idr];
  ...
  if (_scalars_) {
    ...
    if (pba->has_idr == _TRUE_) {
      if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off) {
        for (n_idr=0; n_idr<pba->N_idr; n_idr++) { //AD
            delta_idr[n_idr] = y[ppw->pv->index_pt_delta_idr+n_idr];
            theta_idr[n_idr] = y[ppw->pv->index_pt_theta_idr+n_idr];
        
            if (ppt->idr_nature == idr_free_streaming){
               shear_idr[n_idr] = y[ppw->pv->index_pt_shear_idr+n_idr];
            }
        }
      }
      else{
    ...
    if (pba->has_idr == _TRUE_) {
     for(n_idr=0; n_idr<pba->N_idr; n_idr++){ //AD
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_idr+n_idr]*delta_idr[n_idr];
      ppw->rho_plus_p_theta += 4./3.*ppw->pvecback[pba->index_bg_rho_idr+n_idr]*theta_idr[n_idr];
      if (ppt->idr_nature==idr_free_streaming)
        ppw->rho_plus_p_shear += 4./3.*ppw->pvecback[pba->index_bg_rho_idr+n_idr]*shear_idr[n_idr];
      ppw->delta_p += 1./3. * ppw->pvecback[pba->index_bg_rho_idr+n_idr]*delta_idr[n_idr];
      rho_plus_p_tot += 4./3. * ppw->pvecback[pba->index_bg_rho_idr+n_idr];
     }
    }
...
int perturb_sources(
  ...
  if (_scalars_) {
	...
	if (ppt->has_Nbody_gauge_transfers == _TRUE_){
      fprintf(stderr,"check Nbody gauge \n");   
    ...
    /* delta_idr */ //AD: We don't need N-body gauge correction for the time being
    ...
    /* theta_idr */ //AD: We don't need N-body guage correction for the time being
...
int perturb_print_variables(double tau,
  ...
  double *delta_idr=NULL, *theta_idr=NULL, *shear_idr=NULL, *delta_p_over_delta_rho_idr=NULL;
  int n_idr;
  ...
  a_prime_over_a = pvecback[pba->index_bg_H] * pvecback[pba->index_bg_a];//*SG
  ...
  if (pba->has_idr == _TRUE_){
        class_alloc(delta_idr, sizeof(double)*pba->N_idr,error_message);
        class_alloc(theta_idr, sizeof(double)*pba->N_idr,error_message);
        class_alloc(shear_idr, sizeof(double)*pba->N_idr,error_message);
        class_alloc(delta_p_over_delta_rho_idr, sizeof(double)*pba->N_idr,error_message);
  }
  ...
  if (_scalars_) {
    ...
    if (pba->has_idr == _TRUE_) {
      if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off) {
       for(n_idr=0; n_idr<pba->N_idr; n_idr++){ //AD
        ...
        if(ppt->idr_nature == idr_free_streaming){
          if((pba->has_idm_dr == _TRUE_)&&(ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_on)){
            shear_idr[n_idr] = ppw->tca_shear_idm_dr;
          }
          else{
            shear_idr[n_idr] = y[ppw->pv->index_pt_shear_idr+n_idr];
          }
       ...
       for(n_idr=0; n_idr<pba->N_idr; n_idr++){
           if (ppt->gauge == newtonian) {
        delta_idr[n_idr] = -4.*y[ppw->pv->index_pt_phi];//ppw->rsa_delta_idr[n_idr];//*SG
        theta_idr[n_idr] = 6.*ppw->pvecmetric[ppw->index_mt_phi_prime];//ppw->rsa_theta_idr[n_idr];//*SG
           }
           if (ppt->gauge == synchronous) {
       delta_idr[n_idr] = 4./(k*k)*(a_prime_over_a*ppw->pvecmetric[ppw->index_mt_h_prime] -k*k*y[ppw->pv->index_pt_eta]);//ppw->rsa_delta_idr[n_idr];//*SG
       theta_idr[n_idr] = -0.5*ppw->pvecmetric[ppw->index_mt_h_prime];//ppw->rsa_theta_idr[n_idr];//*SG
           }
        shear_idr[n_idr] = 0.;
       }
      }
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_idr == _TRUE_) {
        for(n_idr=0; n_idr<pba->N_idr; n_idr++){ //AD
            delta_idr[n_idr] -= 4. * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
            theta_idr[n_idr] += k*k*alpha;
        }
      }
    ...
    /* Interacting dark radiation */
    for(n_idr=0; n_idr<pba->N_idr; n_idr++){ //AD
       class_store_double(dataptr, delta_idr[n_idr], pba->has_idr, storeidx);
       class_store_double(dataptr, theta_idr[n_idr], pba->has_idr, storeidx);
       if ((pba->has_idr==_TRUE_) && (ppt->idr_nature == idr_free_streaming))
          class_store_double(dataptr, shear_idr[n_idr], _TRUE_, storeidx);
    }
    /* Interacting dark matter */
...
//*SG
    if (pba->has_idr == _TRUE_){
        ...
        free(theta_idr);
        free(shear_idr);
        free(delta_p_over_delta_rho_idr);
    }
...
int perturb_derivs(double tau,
  ...
  double a,a2,a_prime_over_a,z,R;
  ...
  double Sinv=0., dmu_idm_dr=0., tca_slip_idm_dr=0.;
 ...
 /** - get metric perturbations with perturb_einstein() */
  ...
  N_coupling_idr=pba->N_idr; //AD
  double delta_idr[pba->N_idr]; 
  double theta_idr[pba->N_idr];
  double dmu_idr[N_coupling_idr];
  for(i=0; i<N_coupling_idr; i++){
     delta_idr[i] = 0.;
     theta_idr[i] = 0.;
     dmu_idr[i] = pvecthermo[pth->index_th_dmu_idr+i];
  }
  ...
  if (_scalars_) {
    ...
    if (pba->has_idr == _TRUE_){
        if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off){
           for( n_idr = 0; n_idr < pba->N_idr; n_idr++){    //AD
              delta_idr[n_idr] = y[pv->index_pt_delta_idr + n_idr];
              theta_idr[n_idr] = y[pv->index_pt_theta_idr + n_idr];
           }
        }
    }
    ...
    if (pba->has_idr == _TRUE_){
       if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_on){
          for (n_idr = 0; n_idr < pba->N_idr; n_idr++){    //AD
              //delta_idr[n_idr] = ppw->rsa_delta_idr[n_idr];//*SG
              //theta_idr[n_idr] = ppw->rsa_theta_idr[n_idr];//*SG
              if (ppt->gauge == newtonian) {
                  delta_idr[n_idr] = -4.*y[ppw->pv->index_pt_phi];//ppw->rsa_delta_idr[n_idr];//*SG
                  theta_idr[n_idr] = 6.*ppw->pvecmetric[ppw->index_mt_phi_prime];//ppw->rsa_theta_idr[n_idr];//*SG
              }
              if (ppt->gauge == synchronous) {
                  delta_idr[n_idr] = 4./k2*(a_prime_over_a*ppw->pvecmetric[ppw->index_mt_h_prime] -k2*y[ppw->pv->index_pt_eta]);//ppw->rsa_delta_idr[n_idr];//*SG
                  theta_idr[n_idr] = -0.5*ppw->pvecmetric[ppw->index_mt_h_prime];//ppw->rsa_theta_idr[n_idr];//*SG
              }
          }
       }
    ...
    /** - ---> idr */
    if (pba->has_idr == _TRUE_){
        if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off) {  //AD
             for (n_idr = 0; n_idr < pba->N_idr; n_idr++)  {
                 dy[pv->index_pt_delta_idr+n_idr] = -4./3.*(theta_idr[n_idr] + metric_continuity);
             }
        }
    }
    
    /** - ---> idm_dr */  //AD: commented out to avoid compilation error
    //if (pba->has_idm_dr == _TRUE_){
    ...
    if (pba->has_idr == _TRUE_){
      ...
      if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off) {
        ...
        if ((pba->has_idm_dr == _FALSE_)||((pba->has_idm_dr == _TRUE_)&&(ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_off))) { //AD: This if check is not needed now
         for (n_idr = 0; n_idr < pba->N_idr; n_idr++)  {  //AD: Nothing changed in the _idm_dr parts

          /** - ----> idr velocity */
          if(ppt->idr_nature == idr_free_streaming)
            dy[pv->index_pt_theta_idr+n_idr] = k2*(y[pv->index_pt_delta_idr+n_idr]/4.-s2_squared*y[pv->index_pt_shear_idr+n_idr]) + metric_euler;
          else
            dy[pv->index_pt_theta_idr+n_idr] = k2/4. * y[pv->index_pt_delta_idr+n_idr] + metric_euler;
          ...
          if(ppt->idr_nature == idr_free_streaming){
            ...
            /** - ----> exact idr shear */
            l = 2;
            dy[pv->index_pt_shear_idr+n_idr] = 0.5*(8./15.*(y[pv->index_pt_theta_idr+n_idr]+metric_shear)-3./5.*k*s_l[3]/s_l[2]*y[pv->index_pt_shear_idr+pba->N_idr+n_idr]) - ppt->beta_idr[l-2]*dmu_idr[n_idr]*y[pv->index_pt_shear_idr+n_idr];
            //if (pba->has_idm_dr == _TRUE_)
              //dy[pv->index_pt_shear_idr]-= (ppt->alpha_idm_dr[l-2]*dmu_idm_dr + ppt->beta_idr[l-2]*dmu_idr)*y[pv->index_pt_shear_idr];

            /** - ----> exact idr l=3 */
            l = 3;
            dy[pv->index_pt_l3_idr+n_idr] = k/(2.*l+1.)*(l*2.*s_l[l]*s_l[2]*y[pv->index_pt_shear_idr+n_idr]-(l+1.)*s_l[l+1]*y[pv->index_pt_l3_idr+pba->N_idr+n_idr]) - ppt->beta_idr[l-2]*dmu_idr[n_idr]*y[pv->index_pt_l3_idr+n_idr];
            //if (pba->has_idm_dr == _TRUE_)
            ...
            for (l = 4; l < pv->l_max_idr; l++) {
              dy[pv->index_pt_delta_idr + l*pba->N_idr + n_idr] = k/(2.*l+1)*(l*s_l[l]*y[pv->index_pt_delta_idr+(l-1)*pba->N_idr+n_idr]-(l+1.)*s_l[l+1]*y[pv->index_pt_delta_idr+(l+1)*pba->N_idr+n_idr]) - ppt->beta_idr[l-2]*dmu_idr[n_idr]*y[pv->index_pt_delta_idr+l*pba->N_idr+n_idr];
              //if (pba->has_idm_dr == _TRUE_)
            ...
            /** - ----> exact idr lmax_dr */
            l = pv->l_max_idr;
            dy[pv->index_pt_delta_idr+l*pba->N_idr+n_idr] = k*(s_l[l]*y[pv->index_pt_delta_idr+(l-1)*pba->N_idr+n_idr]-(1.+l)*cotKgen*y[pv->index_pt_delta_idr+l*pba->N_idr+n_idr]) - ppt->beta_idr[l-2]*dmu_idr[n_idr]*y[pv->index_pt_delta_idr+l*pba->N_idr+n_idr];
            //if (pba->has_idm_dr == _TRUE_)
              //dy[pv->index_pt_delta_idr+l]-= (ppt->alpha_idm_dr[l-2]*dmu_idm_dr + ppt->beta_idr[l-2]*dmu_idr)*y[pv->index_pt_delta_idr+l];
          }
         }
        }
        /* with TCA in idm-dr sector */
        //else{
...
int perturb_rsa_idr_delta_and_theta(
  ...
  int n_idr;
  k2 = k*k;
  class_alloc(ppw->rsa_delta_idr, sizeof(double)*pba->N_idr,"idr RSA error");
  class_alloc(ppw->rsa_theta_idr, sizeof(double)*pba->N_idr,"idr RSA error");
  ...
  if (ppt->gauge == newtonian) {
    ...
    if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_on) {
     ...
     for(n_idr=0; n_idr<pba->N_idr; n_idr++){ //AD
      ppw->rsa_delta_idr[n_idr] = -4.*y[ppw->pv->index_pt_phi];
      ppw->rsa_theta_idr[n_idr] = 6.*ppw->pvecmetric[ppw->index_mt_phi_prime];
     }
  ...
  if (ppt->gauge == synchronous) {
    ...
    if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_on) {
     ...
     for(n_idr=0; n_idr<pba->N_idr; n_idr++){ //AD
      ppw->rsa_delta_idr[n_idr] = 4./k2*(a_prime_over_a*ppw->pvecmetric[ppw->index_mt_h_prime]
                                  -k2*y[ppw->pv->index_pt_eta]);
      ppw->rsa_theta_idr[n_idr] = -0.5*ppw->pvecmetric[ppw->index_mt_h_prime];
     }
