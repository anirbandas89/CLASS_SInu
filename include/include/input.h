...
#define class_read_list_of_doubles_one_of_two(name1,name2,destination)        \
  do {                                    \
    class_call(parser_read_list_of_doubles(pfc,name1,&entries_read1,       \
                                           &param1,&flag1,errmsg),    \
            errmsg,                            \
            errmsg);                            \
    class_call(parser_read_list_of_doubles(pfc,name2,&entries_read2,        \
                                           &param2,&flag2,errmsg),    \
            errmsg,                            \
            errmsg);                            \
    class_test((flag1 == _TRUE_) && (flag2 == _TRUE_),            \
            ...
            name1,name2);                        \
    if (flag1 == _TRUE_)                        \
        class_alloc(destination,entries_read1*sizeof(double),errmsg);        \
    if (flag2 == _TRUE_)                        \
        class_alloc(destination,entries_read2*sizeof(double),errmsg);        \
  } while(0);
